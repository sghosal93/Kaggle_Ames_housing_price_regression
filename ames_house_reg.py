...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 22 15:55:25 2019

@author: samghosal
"""

"""README: Download train.csv, test.csv and data_description.txt files from Kaggle first.
      Each sub-section within the code has it's own mini-descriptions. Please check those.
      The code follows a smooth and easily understandable workflow for dealing with the dataset""" 
...
# Download the dataset first. Load train and Test set
...
# Extract and keep the 'Id' column aside
train_ID = train['Id']
test_ID = test['Id']
...
# Check the first few rows and first and last columns of training and test data
train.head()
test.head()

######################################################
# Checking the target prediction variable: SalePrice #
######################################################

# Get Sale Price Description, distribution (try fitting a Gaussian)
...
#######################################################
# Check types of data: Numerical and Categorical Data #
#######################################################
...
####################################################
# Find variables highly correalted with SalesPrice #
####################################################
...
#########################################################################################
# Check distribution of top 10 corr. variables wrt Saleprice and remove outliers if any #
###################### A Bit time consuming, but needs to be done ####################### 
#########################################################################################
...
# get missing data information from "data_description.txt" and fill in missing values appropiately
...
######################################################################
# Transform Numerical (but actually categorical data) to Categorical # 
######################################################################
...
# crafting new feature: Total Square Feet area 
all_data['TotalSF'] = all_data['TotalBsmtSF'] + all_data['1stFlrSF'] + all_data['2ndFlrSF']

#############################################################
# Log transforming the skewed data: getting rid of skewness #
#### (Data follows log-normal dist. - use log-transform) ####
#############################################################
...
###############################
# Craft models for prediction #
###############################

# separate processed train and test data

train = all_data[:ntrain]
test = all_data[ntrain:]

# Build a k-fold cross validation evaluator
n_folds = 5 # k = 5
...
# function for evaluating prediction scores
def rmsle(y, y_pred):
...
# Do a weighted average of Stacked, XGB and LGBM
Stacked = 1/(0.1077)
XGBoost = 1/(0.1177)
LGBM = 1/(0.1159)
...
# ENSEMBLE AVERAGING (Final Model)
ensemble = stacked_pred*Stacked + xgb_pred*XGBoost + lgb_pred*LGBM

sub = pd.DataFrame()
sub['Id'] = test_ID
sub['SalePrice'] = ensemble
sub.to_csv('SP_predictions.csv',index=False)
